...
def create_dataloader(configs, evaluation=False, sort=True):
  ...
  if configs.data.dataset == 'Object5':
    ...
    train_dataset = AAPM(Path(configs.data.root) / f'np_256_img_dataset', sort=False) #data.root = '/media/harry/tomo/AAPM_data/256/train'
    val_dataset = AAPM(Path(configs.data.root) / f'np_256_img_dataset_test', sort=True) #data.root = '/media/harry/tomo/AAPM_data/256/test'
